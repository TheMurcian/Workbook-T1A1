...
Data types are a means of classifying data and informing the computer how we want to handle specific data in one’s program, as well as a means of storing data inside of variables (Hogan, 2017). The most important of them which are native to Ruby include text which is held together in strings; whole and decimal numbers such as integers and floats, respectively, as well as symbols and nil - that is, no value. Consider the following examples:
- name = “Dylan” (string)
...
### Menu class:
- Handles menu option effectively and removes the need to staff/waiters
- Provides details of name, description, and price of items, allowing customers to browse, select and decide which meals to buy
...
### Payment class:
- Handles transactions to allow users to make purchases without having to call in or visit store in person
- To allow a variety of purchasing options eg paypal, credit card, voucher, etc and allow customers to select their preferred option

### Delivery class:
- Handles coordinating and user-driver features 
- Provides details of each delivery driver and vehicle for users to see who, when, and in what vehicle their food will be delivered (getter and setter method)
...
### Explanation:
Celsius in line 2 requires the .to_f method to convert from a string it into a float in order for the program to run. This allows us to input and output floating numbers to accommodate appropriate and accurate conversion rates from celsius to Fahrenheit.  
